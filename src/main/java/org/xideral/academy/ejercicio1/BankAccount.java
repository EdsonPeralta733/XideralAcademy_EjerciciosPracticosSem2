...
public class BankAccount {
    ...
    public void withdraw(double amount) throws InsufficientBalanceException {
        ...
        if (amount > balance) {
            ...
            throw new InsufficientBalanceException(String.format("Fondos insuficientes para retirar $%.2f", amount), deficit);
        }
    ...
    public void transfer(BankAccount target, double amount) throws InsufficientBalanceException {
        // TODO: usar try-with-resources con TransactionLog dentro: withdraw, target.deposit, log ambas operaciones
        try (TransactionLog tranLog = new TransactionLog()) {
            withdraw(amount);
            tranLog.log(String.format("Retiro de $%.2f de cuenta origen. Saldo: $%.2f", amount, balance));
            target.deposit(amount);
            tranLog.log(String.format("Deposito de $%.2f en cuenta destino. Saldo: $%.2f", amount, target.getBalance()));
        }
    }
