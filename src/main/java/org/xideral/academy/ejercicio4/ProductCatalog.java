...
record Product(String name, double price, String category, boolean inStock) {
    ...
    String toDisplayString() {
        return String.format("%-15s $%7.2f  %-12s [%s]", name, price, category, inStock ? "En stock" : "Agotado");
    }
}
...
public class ProductCatalog {
    public static void main(String[] args) {
        ...
        catalogo.stream().map(Product::toDisplayString).forEach(System.out::println);
        ...
        System.out.println("\nTotal disponibles: " + new ProductPipeline().where(Product::isAvailable).count(catalogo));
    }
}
