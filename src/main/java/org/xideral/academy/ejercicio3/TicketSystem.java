...
enum Priority {
    ...
    public String getLabel() {
        ...
        return String.format("%s (Nivel %d, Respuesta: %dh)", name(), level, responseTimeHours);
    }
}
...
class Ticket {
    ...
    public String toString() {
        return String.format("Ticket{id=%d, desc='%s', priority=%s, status=%s}", id, description, priority.getLabel(), status);
    }
}
...
public class TicketSystem {
    public static void main(String[] args) {
        ...
        conteo.forEach((status, count) -> System.out.printf("  %s: %d%n", status, count));
        ...
        tickets.stream().filter(t -> urgentes.contains(t.getPriority())).forEach(System.out::println);
    }
}